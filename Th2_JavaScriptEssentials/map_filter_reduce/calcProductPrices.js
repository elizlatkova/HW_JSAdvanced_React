...
function calcTotalPrice(products) {
    ...
    let total = pricesArr.reduce((acc, price) => acc = acc + price, 0)
    return total
}
...
function calcTotalPriceByCategory(products, category) {
    // YOUR CODE HERE
    let categoryItem = products.filter(item => item.category == category);
    let totalByCategory = categoryItem.reduce((acc, item) => acc += item.price, 0);
